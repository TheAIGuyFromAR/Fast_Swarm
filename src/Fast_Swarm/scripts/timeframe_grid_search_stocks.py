"""
Timeframe Grid Search for STOCKS: Find optimal TF combinations for Bear Protection.

Same methodology as crypto grid search but using Yahoo Finance stock data.
Tests different timeframe pairs for the AJ signal (acc + ADX_jerk):
- Baseline: 1h + 4h
- Candidates: 1h/6h, 4h/6h, 6h/1d, 1h/1d, 4h/1d
...
# AJ config thresholds (same as crypto)
ACC_THRESH = -1.5
JERK_THRESH = -0.5
...
# Stock symbols (downloaded from Yahoo)
SYMBOLS = [
...
def load_stock_tf(symbol: str, timeframe: str) -> dict:
    """Load stock data for a specific timeframe, indexed by time."""
    path = DATA_DIR / f"symbol={symbol}" / f"timeframe={timeframe}" / "data.parquet"
    if not path.exists():
        return None
...
def check_defensive_aj(row: dict) -> bool:
    ...
    jerk = row.get("adx_14_jerk_zscore")  # lowercase, same as crypto
...
def backtest_tf_pair(symbol: str, tf1: str, tf2: str) -> dict:
    ...
    tf1_data = load_stock_tf(symbol, tf1)
    tf2_data = load_stock_tf(symbol, tf2)
...
def run_timeframe_grid():
    print("="*100)
    print("STOCK TIMEFRAME GRID SEARCH: Finding Optimal TF Combinations")
    print("="*100)
    print()
    ...
    available_symbols = []
    for sym in SYMBOLS:
        path = DATA_DIR / f"symbol={sym}" / "timeframe=1h" / "data.parquet"
        if path.exists():
            available_symbols.append(sym)

    print(f"Found data for {len(available_symbols)} symbols")
    print()
    ...
    for tf1, tf2 in TF_PAIRS:
        ...
        if pair_results:
            ...
            print(f"OK ({len(pair_results)} symbols, {win_count} wins, avg ROI diff: {avg_roi:+.1f}%)")
        else:
            print("No data")
    ...
    print("="*100)
    print()
    print(f"{'TF Pair':<12} {'Avg ROI':>12} {'Win':>8} {'Win%':>8} {'Avg DD':>12} {'Def%':>8}")
    print("-"*70)
    ...
    for tf1, tf2 in TF_PAIRS:
        ...
        if pair_results:
            avg_roi = statistics.mean([r["roi_diff"] for r in pair_results])
            win_count = sum(1 for r in pair_results if r["roi_diff"] > 0)
            win_pct = win_count / len(pair_results) * 100
            ...
            print(f"{pair_name:<12} {avg_roi:>+11.1f}% {win_count:>5}/{len(pair_results):<2} {win_pct:>7.1f}% {avg_dd:>+11.1f}% {avg_def:>7.1f}%")
    ...
    for rank, (pair_name, summary) in enumerate(sorted_pairs, 1):
        ...
        print(f"  {rank}. {pair_name:<10}: {summary['avg_roi']:>+6.1f}% avg ROI diff, {summary['win_pct']:>5.1f}% win rate  {status}")
    ...
    # Top 5 best individual stocks per TF pair
    print()
    print("="*100)
    print("TOP 5 STOCKS BY TF PAIR (ROI DIFF)")
    print("="*100)
    print()

    for tf1, tf2 in TF_PAIRS[:3]:  # Top 3 TF pairs
        pair_name = f"{tf1}/{tf2}"
        pair_results = [r for r in all_results if r["tf_pair"] == pair_name]
        if pair_results:
            sorted_stocks = sorted(pair_results, key=lambda x: x["roi_diff"], reverse=True)[:5]
            print(f"{pair_name}:")
            for r in sorted_stocks:
                print(f"    {r['symbol']}: {r['roi_diff']:+.1f}% ROI diff, {r['dd_diff']:+.1f}% DD diff")
            print()

    # Final recommendation
    print()
    print("="*100)
    print("RECOMMENDATION FOR STOCKS")
    print("="*100)
    print()
    ...
    if sorted_pairs:
        ...
        if best_pair == "1h/4h":
            print(f"KEEP BASELINE: 1h/4h is optimal for stocks")
            print(f"  Avg ROI diff: {best_summary['avg_roi']:+.1f}%")
            print(f"  Win rate: {best_summary['win_pct']:.1f}%")
            ...
            print(f"  Win rate: {best_summary['win_pct']:.1f}% (vs {baseline_summary.get('win_pct', 0):.1f}% baseline)")
            print(f"  Improvement: {best_summary['avg_roi'] - baseline_summary.get('avg_roi', 0):+.1f}%")
        else:
            print(f"MARGINAL: {best_pair} slightly better but 1h/4h more consistent")
            print(f"  Recommend sticking with 1h/4h for stability")

    # Compare to crypto results
    print()
    print("="*100)
    print("CRYPTO VS STOCKS COMPARISON")
    print("="*100)
    print()
    print("Crypto 1h/4h baseline: +994.0% avg ROI diff, 100% win rate (3/3)")
    if "1h/4h" in tf_summaries:
        s = tf_summaries["1h/4h"]
        print(f"Stock  1h/4h baseline: {s['avg_roi']:+.1f}% avg ROI diff, {s['win_pct']:.1f}% win rate ({s['win_count']}/{s['total']})")
